...
async def seed_data():
    ...
    devices_data = [
        ...
        ("6152608110001", "6152608110001", "Y6B", merchant_ids[0], "-1001234567890", "ACTIVE", "100%", "Excellent", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q1", "Installed at counter 1", now - timedelta(minutes=2)),
        ("6152608110002", "6152608110002", "Y6B", merchant_ids[1], "-1001987654321", "ACTIVE", "95%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q1", "Installed at main register", now - timedelta(minutes=8)),
        ("6152608110003", "6152608110003", "Y6B", merchant_ids[2], "-1001555444333", "ACTIVE", "88%", "Excellent", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q1", "Cashier station 2", now - timedelta(minutes=15)),
        ("6152608110004", "6152608110004", "Y6B", merchant_ids[3], "-1001777888999", "ACTIVE", "72%", "Moderate", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q2", "Bakery front counter", now - timedelta(hours=1)),
        ("6152608110005", "6152608110005", "Y6B", merchant_ids[4], "-1001112223334", "ACTIVE", "100%", "Excellent", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q2", "24/7 store counter", now - timedelta(minutes=1)),
        ("6152608110006", "6152608110006", "Y6B", merchant_ids[6], "-1001999000111", "ACTIVE", "82%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q2", "Souvenir checkout desk", now - timedelta(hours=3)),
        ("6152608110007", "6152608110007", "Y6B", merchant_ids[7], "-1001888333222", "ACTIVE", "91%", "Excellent", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q2", "Electronics front store", now - timedelta(minutes=12)),
        ("6152608110008", "6152608110008", "Y6B", merchant_ids[8], "-1001444555666", "ACTIVE", "64%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q2", "Coffee bar counter", now - timedelta(hours=5)),

        # --- B. Warehouse In-Stock Units (Unassigned & Ready for Deployment) ---
        ("6152608110010", "6152608110010", "Y6B", None, None, "IN_STOCK", "100%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q3", "Warehouse Shelf A-01 (Tested)", now - timedelta(days=2)),
        ("6152608110011", "6152608110011", "Y6B", None, None, "IN_STOCK", "100%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q3", "Warehouse Shelf A-02 (Tested)", now - timedelta(days=2)),
        ("6152608110012", "6152608110012", "Y6B", None, None, "IN_STOCK", "100%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q3", "Warehouse Shelf A-03 (Tested)", now - timedelta(days=2)),
        ("6152608110013", "6152608110013", "Y6B", None, None, "IN_STOCK", "100%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q3", "Warehouse Shelf A-04 (Tested)", now - timedelta(days=2)),
        ("6152608110020", "6152608110020", "Y6_LCD", None, None, "IN_STOCK", "100%", "Good", "Y6_LCD_1605_V1.0", "esp32c2x_2M_OTA", "BATCH-2026-Q4", "Warehouse Shelf B-01 (Factory New)", now - timedelta(days=1)),
        ...
        if existing_dev:
            ...
            await conn.execute("""
                ...
                SET device_id = $2, device_sn = $3, device_model = $4, merchant_id = $5,
                    ...
                    batch_no = $13, notes = $14,
                    last_online = $15, last_heartbeat = $15, updated_at = CURRENT_TIMESTAMP
                WHERE id = $1
            """, d_pk, d_id, sn, model, mid, chat_id, st_val, is_active, batt, sig, v4g, vwifi, b_no, notes, ltime)
        else:
            dev_row = await conn.fetchrow("""
                INSERT INTO devices (device_id, device_sn, device_model, merchant_id, telegram_chat_id, chat_id,
                                    status, is_active, battery, signal, version_4g, version_wifi, batch_no, notes,
                                    last_online, last_heartbeat)
                VALUES ($1, $2, $3, $4, $5, $5, $6::device_status, $7, $8, $9, $10, $11, $12, $13, $14, $14)
                RETURNING id
            """, d_id, sn, model, mid, chat_id, st_val, is_active, batt, sig, v4g, vwifi, b_no, notes, ltime)
            d_pk = dev_row["id"]
        dev_db_map[sn] = d_pk
